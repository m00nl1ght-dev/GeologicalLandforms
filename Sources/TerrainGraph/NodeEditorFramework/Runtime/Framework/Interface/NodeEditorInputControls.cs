...
namespace NodeEditorFramework 
{
	...
	public static class NodeEditorInputControls
	{
		...
		[ContextFillerAttribute (ContextType.Node)]
		private static void FillNodeActions (NodeEditorInputInfo inputInfo, GenericMenu canvasContextMenu) 
		{
			if (inputInfo.editorState.focusedNode != null) 
			{
				inputInfo.editorState.focusedNode.FillNodeActionsMenu(inputInfo, canvasContextMenu);
			}
		}
		...
		private static void DeleteNodeKey(NodeEditorInputInfo inputInfo)
		{
			if (GUIUtility.keyboardControl > 0)
				return;
			NodeEditorState state = inputInfo.editorState;
			if (inputInfo.editorState.focusedNode != null && state.canvas.CanDeleteNode(state.focusedNode))
			{
