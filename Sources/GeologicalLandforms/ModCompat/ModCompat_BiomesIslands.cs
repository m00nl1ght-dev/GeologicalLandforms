...
internal static class ModCompat_BiomesIslands
{
    ...
    static ModCompat_BiomesIslands()
    {
        try
        {
            ...
            if (biType != null)
            {
                Log.Message(ModInstance.LogPrefix + "Applying integration with Biomes! Islands.");

                RimWorld_GenStep_Terrain.ApplyBiomeReplacements += ApplyBiomeReplacements;
        ...
        {
            Log.Error(ModInstance.LogPrefix + "Failed to apply integration with Biomes! Islands!");
        }
    }
